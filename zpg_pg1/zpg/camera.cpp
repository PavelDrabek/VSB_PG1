...
{		
	Vector3 axis_z_ = view_at_ - view_from_;
	axis_z_.Normalize();
...
Ray Camera::GenerateRay(const float sx, const float sy) const
{
	...
	Vector3 direction = -Vector3(x, y, -d_); // smer noveho paprsku v kamerovem prostoru	
	//direction.Normalize();
...
/*
Vector3 lambertShader(Vector3 point, Vector3 normal, Vector3 cameraPos) {
	Vector3 lightDir = point - cameraPos;
	lightDir.Normalize();
...
Vector3 normalShader(Vector3 normal) {
	return ((normal * 0.5f) + Vector3(0.5f, 0.5f, 0.5f));
}

int Camera::Render(RTCScene &scene, std::vector<Surface *> & surfaces)
{
	cv::Vec3f c;
	for (int y = 0; y < height_; y++) {
		for (int x = 0; x < width_; x++) {
			Ray rtc_ray = GenerateRay(x, y);
			//rtc_ray.tfar = 1.5;	
			...
			if (rtc_ray.geomID != RTC_INVALID_GEOMETRY_ID) {
				Vector3 n = rtc_ray.Ng;
				n.Normalize();
				//Vector3 color = normalShader(-n);
				Vector3 color = lambertShader(rtc_ray.eval(rtc_ray.tfar), n, view_from_);
				c = cv::Vec3f(color.x, color.y, color.z);
			} else {
				c = cv::Vec3f(0, 0, 0);
			}
			src_32fc3_img.at<cv::Vec3f>(y, x) = c; // cv::Vec3b((uchar)x, (uchar)y, (uchar)255);
		}
	}
	cv::imshow("Raytracer", src_32fc3_img); // display image
